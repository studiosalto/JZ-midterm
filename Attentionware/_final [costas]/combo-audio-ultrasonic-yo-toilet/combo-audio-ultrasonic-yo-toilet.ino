...
void loop_ultrasound() {
  ...
  if (readingsCount >= minReqTriggers) {    
    ...
    else if (ravg.getAverage() > /*65*/ 90 && ravg.getAverage() < /*175*/ 120) {
      myState=2;
      on_trigger();     
    }
    
    //myState 1: Noone is in the room but I mumble now
    else if (ravg.getAverage() >= /*178*/ 131 /*&& ravg.getAverage() < /* 180 */ /*132*/) { // NATURAL delay due to random nosie. the && are used for safety 
